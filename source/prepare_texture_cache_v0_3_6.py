...
def parse_txtr(path):
    ...
    for i,p in enumerate(ptrs):
        if p==0:
            pages.append(dict(page=i, external=True, width=0,height=0,blobOffset=0,blobSize=0)); continue
        if p+28 > len(b): raise RuntimeError(f'TXTR page {i}: entry fora do arquivo.')
        w,h=i32(b,p+12),i32(b,p+16); off=u32(b,p+24)
        if not (1<=w<=4096 and 1<=h<=4096): raise RuntimeError(f'TXTR page {i}: dimensoes invalidas {w}x{h}.')
        pages.append(dict(page=i, external=(off==0), width=w,height=h,blobOffset=off,blobSize=0))
